...
class EntityBase:
    ...
    def __str__(self):
        ...
        return json.dumps(
            {
                "str": self.baseStrength,
                "agi": self.baseAgility,
                "int": self.baseIntellect,
                "Gender": self.gender,
                "Name": self.name,
                "Slots": slots,
                "Health": self.health,
    ...
    def additional_damage(self, item, damage):
        ...
        if attr == "strength":
            ...
            return 0
        return int((1 + (random.randrange(90, 111, 1) * stat) / 10000.0) * damage)
    ...
    def equip_weapon(
        self, item, slot=True
    ):  # Slot: True == 0, False == 1 aka True = RHand, False = LHand
        if item.twoH:
            changed = 0
...
class Player(EntityBase):
    ...
    def get_level(self):
        l = self.level
        while self.exp > -(l ** m.e) * (
            m.e ** (1 - l - m.pi) * (l + m.e) ** m.e - m.e ** (1 + m.e - m.pi)
        ):
            l += 1
        return l - 1
    ...
    def level_up(self, level_dif):
        ...
        print(
            "Max. HP  "
            + s_print("+" + str(hp_gain), 4)
            + s_print(str(self.max_health), 6)
        )
        print(
            "Agility  "
            + s_print("+" + str(2 * level_dif), 4)
            + s_print(str(self.agility), 6)
        )
        print(
            "Strength "
            + s_print("+" + str(2 * level_dif), 4)
            + s_print(str(self.strength), 6)
        )
        print(
            "intellect"
            + s_print("+" + str(2 * level_dif), 4)
            + s_print(str(self.intellect), 6)
        )
